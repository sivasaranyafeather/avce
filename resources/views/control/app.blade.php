...
<head>
  ...
  <style>
     table.dataTable tbody tr {
        border: 1px solid #dee2e6; /* Or any color that matches your theme */
    }
    a {
  ...
  .active{
   ...
   .back-color{
    ...
    .btn-val {
    color: #fff !important;
    background-color: #3d7d91 !important;
    border-color: #3d7d91 !important;
}
.submit_btn {
    color: #fff;
    background-color:#59a8c0;
      ...
      <div class="sidebar">
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item">
              <a href="{{ route('index_admission') }}" class="nav-link @if ($active == 'admission') active @endif">
                <i class="nav-icon fas fa-award"></i>
                <p>
                 Admission
                <span class="badge badge-info right"></span>
  ...
  <script>
    $(document).ready(function() {
      ...
      @if (Session::has('successmessage'))
        toastr.success("{{ Session::get('successmessage') }}", null, { closeButton: true });
      @endif
      
      @if (Session::has('errormessage'))
        toastr.error("{{ Session::get('errormessage') }}", null, { closeButton: true });
      @endif
      
      @if (Session::has('infomessage'))
        toastr.info("{{ Session::get('infomessage') }}", null, { closeButton: true });
      @endif
    });
  </script>
